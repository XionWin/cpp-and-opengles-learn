...
int main()
{
	...
	{
		int pos = i + 1;
		vertices[pos].x = cos(i * (2.0f * M_PI / (size - 2))) * TRIANGLE_SIZE;
		vertices[pos].y = sin(i * (2.0f * M_PI / (size - 2))) * TRIANGLE_SIZE;
		...
		Vertex* vp = vertices + pos;
		float* fp = (float*)vp;
		float* fpp = fp + 2 + (i % 3);
		*fpp = 1.0f;
		// float* fp = (float*)vp;
		// float f = *(fp + sizeof(float) * 2 + sizeof(float) * 0);
		// f = 1.0f;
	}
	...
	sdl2.update(
		[proj_mat_location, model_mat_location, size, indices] () {
			int quit = 0;
			SDL_Event e;
